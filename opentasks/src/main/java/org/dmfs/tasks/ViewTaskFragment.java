...
/**
 * A fragment representing a single Task detail screen. This fragment is either contained in a {@link TaskListActivity}
 * in two-pane mode (on tablets) or in a {@link ViewTaskActivity} on handsets.
 *
        ...
        implements OnModelLoadedListener, OnContentChangeListener, OnMenuItemClickListener, OnOffsetChangedListener
{
    ...
    /**
     * A set of values that may affect the recurrence set of a task. If one of these values changes we have to submit
     * all of them.
     */
    private final static Set<String> RECURRENCE_VALUES = new HashSet<String>(
            Arrays.asList(new String[] {
                    Tasks.DUE, Tasks.DTSTART, Tasks.TZ, Tasks.IS_ALLDAY, Tasks.RRULE, Tasks.RDATE, Tasks.EXDATE }));
    ...
    public interface Callback
    {
         ...
         * @param data
         *         The task data that belongs to the {@link Uri}. This is purely an optimization and may be
         *         <code>null</code>.
         */
         ...
         *
         * @param taskUri
         *         The {@link Uri} of the deleted task. Note that the Uri is likely to be invalid at the time of calling
         *         this method.
         */
    ...
    /**
     * Load the task with the given {@link Uri} in the detail view. <p> At present only Task Uris are supported. </p>
    ...
    /**
     * Update the detail view with the current ContentSet. This removes any previous detail view and creates a new one
     * if {@link #mContentSet} is not <code>null</code>.
     */
    private void updateView()
    {
        ...
        if (mContent != null && activity != null)
        {
            ...
            if (mContentSet != null)
            {
                ...
                {
                    Model minModel = Sources.getInstance(activity)
                            .getMinimalModel(TaskFieldAdapters.ACCOUNT_TYPE.get(mContentSet));
                    mToolbarInfo.setModel(minModel);
                    mToolbarInfo.setValues(null);
                    mToolbarInfo.setValues(mContentSet);
                }
                ((TextView) mToolBar.findViewById(R.id.toolbar_title)).setText(
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater)
    {
        ...
        if (mTaskUri != null)
        {
            ...
            if (mContentSet != null)
            {
                ...
                if (!mShowFloatingActionButton && !(TaskFieldAdapters.IS_CLOSED.get(
                        mContentSet) || status != null && status == Tasks.STATUS_COMPLETED))
                {
    ...
    private void updateColor()
    {
        ...
        if (mShowFloatingActionButton && mFloatingActionButton.getVisibility() == View.VISIBLE)
        {
            ...
            mFloatingActionButton.setBackgroundTintList(
                    new ColorStateList(new int[][] { new int[] { 0 } }, new int[] { Color.HSVToColor(hsv) }));
        }
    }
    ...
    public void onContentLoaded(ContentSet contentSet)
    {
        if (contentSet.containsKey(Tasks.ACCOUNT_TYPE))
        {
            ...
            if (mModel == null || !TextUtils.equals(mModel.getAccountType(),
                    contentSet.getAsString(Tasks.ACCOUNT_TYPE)))
            {
    ...
    {
        return (mOldStatus != -1 && mOldStatus != newStatus || mOldStatus == -1 && TaskFieldAdapters.IS_CLOSED.get(
                mContentSet));
    }
    ...
    {
        AlphaAnimation alphaAnimation = (visibility == View.VISIBLE) ? new AlphaAnimation(0f, 1f) : new AlphaAnimation(
                1f, 0f);
